...
app.post('/api/send-report', async (req, res) => {
    ...
    try {
        await sendEmailReport(url, report, userEmail); // Pass userEmail to the function
...
// ... (Frontend File Serving is unchanged) ...
...
// ... (crawlSite, fetchHtml, processResults are unchanged) ...

// UPDATED to accept userEmail as the recipient
async function sendEmailReport(url, results, userEmail) {
    const { summary, detailedReport, pagesCrawled } = results;
    const { averageScore } = summary;
    let reportHtml = `... (email body is the same) ...`;
    
    const msg = {
        to: userEmail, // Send TO the user
        from: FROM_EMAIL,
        bcc: TO_EMAIL, // BCC you so you get a copy of the lead
        subject: `Your GEO Inspection Report for ${url}`,
        html: reportHtml,
    };

    try {
        await sgMail.send(msg);
        console.log(`Email report for ${url} sent successfully to ${userEmail}.`);
    } catch (error) {
        console.error('Error sending email report:', error.toString());
    }
}

// ... (All other analysis functions are unchanged) ...
...
async function sendEmailReport(url,results,userEmail){const{summary:summary,detailedReport:detailedReport,pagesCrawled:pagesCrawled}=results,{averageScore:averageScore}=summary;let reportHtml=`
    ...
    `;for(const name in detailedReport){const stats=detailedReport[name],passPercent=Math.round(stats.passedCount/stats.totalCount*100),icon=passPercent>=75?"✔️":"❌",color=passPercent>=75?"green":"red";reportHtml+=`<h3 style="color: ${color};">${icon} ${name} - Passed on ${stats.passedCount} of ${stats.totalCount} pages (${passPercent}%)</h3>`,stats.failures.length>0&&(reportHtml+='<p><strong>Details of Omissions/Errors:</strong></p><ul style="list-style-type: none; padding-left: 0;">',stats.failures.forEach(failure=>{reportHtml+=`<li style="margin-bottom: 10px; padding: 10px; background-color: #f8f9fa; border-radius: 4px;">
                ...
                </li>`}),reportHtml+="</ul>"),reportHtml+="<br>"}reportHtml+='</div></body>';const msg={to:userEmail,from:FROM_EMAIL,bcc:TO_EMAIL,subject:`Your GEO Inspection Report for ${url}`,html:reportHtml};try{await sgMail.send(msg),console.log(`Email report for ${url} sent successfully to ${userEmail}.`)}catch(error){console.error("Error sending email report:",error.toString())}}
