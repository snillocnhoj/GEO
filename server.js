...
async function crawlSite(startUrl) {
    ...
    allPageResults.push({ url: startUrl, doc: homePageDoc });
    ...
    for (const url of menuUrlsToCrawl) {
        ...
        try {
            ...
            allPageResults.push({ url: url, doc: pageDoc });
        } catch (error) {
...
// --- NEW "QUOTABILITY" SCORING FUNCTION ---
function calculateQuotabilityScore(allPageDocs) {
    let totalSentences = 0;
    let quotableSentences = 0;

    allPageDocs.forEach(doc => {
        doc.querySelectorAll('p').forEach(p => {
            const sentences = p.textContent.match(/[^.!?]+[.!?]+/g) || [];
            totalSentences += sentences.length;
            sentences.forEach(sentence => {
                const words = sentence.trim().split(/\s+/).length;
                // Criteria for a "quotable" sentence:
                // 1. Between 10 and 30 words long.
                // 2. Starts with a capital letter (a sign of a complete sentence).
                // 3. Doesn't start with a conversational filler word.
                if (words >= 10 && words <= 30 && /^[A-Z]/.test(sentence.trim()) && !/^(But|And|So|Well),/i.test(sentence.trim())) {
                    quotableSentences++;
                }
            });
        });
    ...
    return totalSentences > 0 ? Math.round((quotableSentences / totalSentences) * 100) : 0;
}
...
function processResults(allPageResults) {
    ...
    const allDocs = allPageResults.map(p => p.doc);
    const quotabilityScore = calculateQuotabilityScore(allDocs);

    allPageResults.forEach(pageResult => {
        const checks = runAllChecks(pageResult.doc, pageResult.url);
    ...
    const summary = {
        averageScore,
        quotabilityScore, // Add the new score here
