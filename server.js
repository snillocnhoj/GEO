...
async function crawlSite(startUrl) { /* ... unchanged ... */ }

// --- UPDATED fetchHtml function ---
...
app.post('/api/analyze',apiLimiter,async(req,res)=>{if(!SCRAPER_API_KEY&&"scraperapi"===SCRAPER_SERVICE||!SCRAPINGBEE_API_KEY&&"scrapingbee"===SCRAPER_SERVICE||!SENDGRID_API_KEY||!FROM_EMAIL||!TO_EMAIL)return console.error("One or more environment variables are not set on the server."),res.status(500).json({error:"Server is not configured correctly."});const{startUrl:startUrl}=req.body;if(!startUrl)return res.status(400).json({error:"startUrl is required"});console.log(`Starting analysis for: ${startUrl}`);try{const results=await crawlSite(startUrl),reportId=uuidv4();reportCache.set(reportId,{report:results,url:startUrl}),setTimeout(()=>reportCache.delete(reportId),36e5),res.status(200).json({summary:results.summary,reportId:reportId})}catch(error){console.error(`Analysis failed for ${startUrl}:`,error),res.status(500).json({error:"Failed to complete analysis."})}}),app.post('/api/send-report',async(req,res)=>{const{reportId:reportId,userEmail:userEmail}=req.body;if(!reportId||!reportCache.has(reportId))return res.status(404).send("Report not found or expired.");if(!userEmail)return res.status(400).send("Email address is required.");const{report:report,url:url}=reportCache.get(reportId);try{const origin=`${req.protocol}://${req.get("host")}`;await sendEmailReport(url,report,userEmail,origin),res.status(200).send("Report sent successfully.")}catch(error){res.status(500).send("Failed to send email.")}}),app.get("/style.css",(req,res)=>{res.sendFile(path.join(__dirname,"style.css"))}),app.get("/script.js",(req,res)=>{res.sendFile(path.join(__dirname,"script.js"))}),app.get("/logo.png",(req,res)=>{res.sendFile(path.join(__dirname,"logo.png"))}),app.get("/john-photo.png",(req,res)=>{res.sendFile(path.join(__dirname,"john-photo.png"))}),app.get("/animated-background.gif",(req,res)=>{res.sendFile(path.join(__dirname,"animated-background.gif"))}),app.get("/",(req,res)=>{res.sendFile(path.join(__dirname,"index.html"))});
async function crawlSite(startUrl){const allPageResults=[],siteOrigin=(new URL(startUrl)).origin,homePageHtml=await fetchHtml(startUrl),homePageDom=new JSDOM(homePageHtml),homePageDoc=homePageDom.window.document;allPageResults.push({url:startUrl,checks:runAllChecks(homePageDoc,startUrl)});const crawledUrls=new Set([startUrl]),menuUrlsToCrawl=findMenuLinks(homePageDoc,startUrl,crawledUrls);for(const url of menuUrlsToCrawl){if(crawledUrls.size>=10)break;if(crawledUrls.has(url))continue;try{crawledUrls.add(url);const pageHtml=await fetchHtml(url),pageDom=new JSDOM(pageHtml),pageDoc=pageDom.window.document;allPageResults.push({url:url,checks:runAllChecks(pageDoc,url)})}catch(error){console.error(`Failed to crawl menu page ${url}:`,error.message)}}return processResults(allPageResults)}
function processResults(allPageResults){const detailedReport={};let totalPasses=0,totalChecks=0;allPageResults.forEach(pageResult=>{pageResult.checks.forEach(checkResult=>{const{name:name,passed:passed,details:details}=checkResult;detailedReport[name]||(detailedReport[name]={passedCount:0,totalCount:0,failures:[]}),detailedReport[name].totalCount++,totalChecks++,passed?(detailedReport[name].passedCount++,totalPasses++):detailedReport[name].failures.push({url:pageResult.url,details:details})})});const averageScore=totalChecks>0?Math.round(totalPasses/totalChecks*100):0,summary={averageScore:averageScore,checkStats:Object.fromEntries(Object.entries(detailedReport).map(([name,data])=>[name,{passed:data.passedCount,total:data.totalCount}]))};return{summary:summary,detailedReport:detailedReport,pagesCrawled:allPageResults.length}}
const REPORT_DETAILS={};
async function sendEmailReport(url,results,userEmail,origin){const{summary:summary,detailedReport:detailedReport,pagesCrawled:pagesCrawled}=results,{averageScore:averageScore}=summary;let reportHtml=`
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
            <div style="max-width: 800px; margin: 20px auto; padding: 20px; border: 1px solid #ddd; border-radius: 5px;">
                    ...
                    <tr>
                        ...
                        <td style="padding-left: 20px;">
                            ...
                            <p style="margin-top: 20px;">
                                ...
                                <a href="https://www.johncollins.biz/" target="_blank" style="text-decoration: none; color: #333;">John Collins Consulting</a><br>
                            ...
                            <p style="font-size: 0.9em; color: #555; margin-top: 15px;">
                                <em>P.S. If you need any assistance with your strategic marketing messaging or promotions or any other marketing needs, just reach out. I'm happy to help.</em>
                            </p>
                        </td>
                    </tr>
                </table>
                ...
                <hr>
    `;for(const name in detailedReport){const stats=detailedReport[name],details=REPORT_DETAILS[name]||{what:"N/A",why:"N/A"},passPercent=Math.round(stats.passedCount/stats.totalCount*100),icon=passPercent>=75?"✔️":"❌",color=passPercent>=75?"#2a9d8f":"#e76f51";reportHtml+=`
        ...
        `,stats.failures.length>0&&(reportHtml+='<div style="margin-top: 15px; padding-left: 20px;"><strong>Pages with Omissions/Errors:</strong><ul style="list-style-type: none; padding-left: 0; margin-top: 5px;">',stats.failures.forEach(failure=>{reportHtml+=`<li style="margin-bottom: 10px; padding: 10px; background-color: #f8f9fa; border-radius: 4px;">
                ...
                </li>`}),reportHtml+="</ul></div>"),reportHtml+="</div>"}reportHtml+='</div></body>';const userReportMsg={to:userEmail,from:FROM_EMAIL,replyTo:TO_EMAIL,subject:`Your Detailed GEO Inspection Report for ${url}`,html:reportHtml},leadNotificationMsg={to:TO_EMAIL,from:FROM_EMAIL,subject:`GEO Report Request: ${userEmail} for ${url}`,html:`
            <p>A new GEO report was requested for the website: <strong>${url}</strong></p>
            <p>The user's email is: <strong>${userEmail}</strong></p>
            <hr>
            <h3>Full Report:</h3>
            ${reportHtml}
        `};try{await sgMail.send([userReportMsg,leadNotificationMsg]),console.log(`Email report sent to ${userEmail} and notification sent to ${TO_EMAIL}.`)}catch(error){console.error("Error sending email report:",error.toString())}}
