...
const TO_EMAIL = process.env.TO_EMAIL; // This is your email for receiving notifications
...
app.post('/api/send-report', async (req, res) => {
    ...
    try {
        // Pass the request origin for image URLs
...
async function fetchHtml(url) { /* ... unchanged ... */ }
function processResults(allPageResults) { /* ... unchanged ... */ }
function findMenuLinks(doc, startUrl, crawledUrls) { /* ... unchanged ... */ }
function runAllChecks(doc, url) { /* ... unchanged ... */ }
// ... all individual check functions are also unchanged ...


// --- NEW: In-depth content for the email report ---
...
// --- UPDATED: Email Reporting Function ---
async function sendEmailReport(url, results, userEmail, origin) {
    ...
    // --- Build the new, detailed HTML report for the user ---
    let userReportHtml = `
    ...
    for (const name in detailedReport) {
        ...
        if (stats.failures.length > 0) {
            userReportHtml += `<div style="margin-top: 15px; padding-left: 20px;"><strong>Pages with Omissions/Errors:</strong><ul style="list-style-type: none; padding-left: 0; margin-top: 5px;">`;
            stats.failures.forEach(failure => {
                userReportHtml += `<li style="margin-bottom: 10px; padding: 10px; background-color: #f8f9fa; border-radius: 4px;">
                    <a href="${failure.url}" target="_blank" style="color: #4361ee;">${failure.url}</a><br>
                    <small style="color: #666;"><strong>Issue:</strong> ${failure.details}</small>
                </li>`;
            });
            userReportHtml += `</ul></div>`;
        }
        userReportHtml += `</div>`;
    }
    
    userReportHtml += `</div></body>`;
    
    // --- Define the two separate emails ---

    // 1. The detailed report for the user
    const userReportMsg = {
        to: userEmail,
        from: FROM_EMAIL,
        replyTo: TO_EMAIL, // Sets your email as the reply-to address
        subject: `Your Detailed GEO Inspection Report for ${url}`,
        html: userReportHtml,
    };

    // 2. The simple lead notification for you
    const leadNotificationMsg = {
        to: TO_EMAIL,
        from: FROM_EMAIL,
        subject: `GEO Report Request`,
        html: `<p>A new GEO report was requested for the website: <strong>${url}</strong></p><p>The user's email is: <strong>${userEmail}</strong></p>`,
    };

    try {
        // Send both emails in a single API call
        await sgMail.send([userReportMsg, leadNotificationMsg]);
        console.log(`Email report for ${url} sent successfully to ${userEmail} and notification sent to ${TO_EMAIL}.`);
    } catch (error) {
